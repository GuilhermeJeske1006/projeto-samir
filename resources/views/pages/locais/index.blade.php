...
new class extends Component {
    ...
    {
        return [
            'locais' => Local::query()->when($this->search, fn($q) => $q->where('nome', 'like', "%{$this->search}%"))->orderBy('nome')->paginate(10),
        ];
    }
}; ?>
                        ...
                        <flux:table.row>
                            ...
                            <flux:table.cell>
                                <flux:badge :variant="$local->ativo ? 'success' : 'danger'"
                                    wire:click="toggleStatus({{ $local->id }})" class="cursor-pointer">
                                    {{ $local->ativo ? 'Ativo' : 'Inativo' }}
                                </flux:badge>
                            </flux:table.cell>
                            <flux:table.cell class="flex gap-2">
                                <flux:button size="sm" href="{{ route('locais.edit', $local) }}" wire:navigate
                                    icon="pencil" />
                                <flux:button size="sm" variant="danger" wire:click="delete({{ $local->id }})"
                                    wire:confirm="Tem certeza que deseja excluir este local?" icon="trash" />
                            </flux:table.cell>
                        </flux:table.row>
                    @empty
                        <flux:table.row>
