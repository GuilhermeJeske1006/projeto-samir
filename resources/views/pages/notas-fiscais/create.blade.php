...
new class extends Component {
    ...
    // Edição inline do local
    public bool $editandoLocal = false;
    public string $edit_nome = '';
    public string $edit_cnpj = '';
    public string $edit_razao_social = '';
    public string $edit_endereco = '';
    public string $edit_email = '';
    public string $edit_telefone = '';
    ...
    public function abrirEdicaoLocal()
    {
        $local = Local::find($this->local_id);
        if (!$local) return;

        $this->edit_nome = $local->nome;
        $this->edit_cnpj = $local->cnpj ?? '';
        $this->edit_razao_social = $local->razao_social ?? '';
        $this->edit_endereco = $local->endereco ?? '';
        $this->edit_email = $local->email ?? '';
        $this->edit_telefone = $local->telefone ?? '';
    ...
    public function salvarLocal()
    {
        $this->validate([
            ...
            'edit_cnpj'         => 'nullable|string|max:18',
            'edit_razao_social' => 'nullable|string|max:255',
            'edit_endereco'     => 'nullable|string|max:255',
            'edit_email'        => 'nullable|email|max:255',
            'edit_telefone'     => 'nullable|string|max:20',
        ], [], [
            'edit_nome'         => 'nome',
            'edit_cnpj'         => 'CNPJ',
            'edit_razao_social' => 'razão social',
            'edit_endereco'     => 'endereço',
            'edit_email'        => 'e-mail',
            'edit_telefone'     => 'telefone',
        ]);

        Local::where('id', $this->local_id)->update([
            'nome'         => $this->edit_nome,
            'cnpj'         => $this->edit_cnpj ?: null,
            'razao_social' => $this->edit_razao_social ?: null,
            'endereco'     => $this->edit_endereco ?: null,
            'email'        => $this->edit_email ?: null,
            'telefone'     => $this->edit_telefone ?: null,
        ]);

        $this->editandoLocal = false;
    }

    public function cancelarEdicaoLocal()
    {
        $this->editandoLocal = false;
    }
    ...
    {
        $digits = preg_replace('/\D/', '', $this->edit_cnpj);
        if (strlen($digits) !== 14) {
            $this->cnpjStatus = '';
            return;
        }
        ...
        try {
            ...
            if ($response->successful()) {
                ...
                $this->edit_razao_social = $d['razao_social'] ?? '';
                if (empty($this->edit_nome)) {
                    $this->edit_nome = !empty($d['nome_fantasia']) ? $d['nome_fantasia'] : ($d['razao_social'] ?? '');
                }
                ...
                $this->edit_endereco = implode(', ', $partes);

                if (!empty($d['email'])) {
                    $this->edit_email = strtolower($d['email']);
                }

                $tel = preg_replace('/\D/', '', $d['ddd_telefone_1'] ?? '');
                if (strlen($tel) === 10) {
                    $this->edit_telefone = '(' . substr($tel, 0, 2) . ') ' . substr($tel, 2, 4) . '-' . substr($tel, 6);
                } elseif (strlen($tel) === 11) {
                    $this->edit_telefone = '(' . substr($tel, 0, 2) . ') ' . substr($tel, 2, 5) . '-' . substr($tel, 7);
                }
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-700 p-6">
                ...
                @if ($tipo === 'servico')
                    <div class="space-y-3">
                        ...
                        @if ($editandoLocal)
                            <div class="border border-blue-200 dark:border-blue-800 rounded-lg bg-blue-50/50 dark:bg-blue-900/10 p-4 space-y-4">
                                <p class="text-xs font-semibold text-blue-600 dark:text-blue-400 uppercase tracking-wide">Editar dados do cliente</p>

                                <flux:input wire:model="edit_nome" label="Nome" required />

                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <div>
                                        <flux:input
                                            wire:model.defer="edit_cnpj"
                                        ...
                                        @elseif ($cnpjStatus === 'inactive')
                                            <p class="mt-1.5 text-xs text-amber-600 dark:text-amber-400 flex items-center gap-1">
                                                ...
                                                Situação cadastral não está ativa
                                            </p>
                                    ...
                                    <flux:input wire:model="edit_razao_social" label="Razão Social" />
                                </div>

                                <flux:input wire:model="edit_endereco" label="Endereço" placeholder="Rua, número, bairro, cidade" />

                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <flux:input wire:model="edit_email" label="E-mail" type="email" />
                                    <flux:input wire:model="edit_telefone" x-mask:dynamic="$input.replace(/\D/g,'').length > 10 ? '(99) 99999-9999' : '(99) 9999-9999'" label="Telefone" placeholder="(00) 00000-0000" />
                                </div>

                                <div class="flex gap-2 pt-1">
                                    <flux:button type="button" wire:click="salvarLocal" variant="primary" size="sm">Salvar alterações</flux:button>
                                    <flux:button type="button" wire:click="cancelarEdicaoLocal" variant="ghost" size="sm">Cancelar</flux:button>
                                </div>
                            </div>
                        @endif
                    </div>
