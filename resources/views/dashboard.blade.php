...
new class extends Component {
    ...
    {
        $query = RegistroHora::query()->when($this->local_id, fn($q) => $q->where('local_id', $this->local_id))->when($this->funcionario_id, fn($q) => $q->where('funcionario_id', $this->funcionario_id))->when($this->data_inicio, fn($q) => $q->whereDate('data', '>=', $this->data_inicio))->when($this->data_fim, fn($q) => $q->whereDate('data', '<=', $this->data_fim));
        ...
        $porFuncionario = (clone $query)->select('funcionario_id')->selectRaw('SUM(horas) as total_horas')->selectRaw('SUM(horas * valor_hora_funcionario) as total_pagar')->selectRaw('SUM(CASE WHEN pago_funcionario = 1 THEN horas * valor_hora_funcionario ELSE 0 END) as total_pago')->selectRaw('SUM(CASE WHEN pago_funcionario = 0 THEN horas * valor_hora_funcionario ELSE 0 END) as total_pendente')->groupBy('funcionario_id')->with('funcionario')->get();

        $porLocal = (clone $query)->select('local_id')->selectRaw('SUM(horas) as total_horas')->selectRaw('SUM(horas * valor_hora_funcionario) as total_pagar')->selectRaw('SUM(horas * valor_hora_local) as total_receber')->selectRaw('SUM(CASE WHEN pago_local = 1 THEN horas * valor_hora_local ELSE 0 END) as total_pago')->selectRaw('SUM(CASE WHEN pago_local = 0 THEN horas * valor_hora_local ELSE 0 END) as total_pendente')->groupBy('local_id')->with('local')->get();
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                ...
                <div class="bg-white dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 p-6">
                    ...
                    <div class="mt-2 text-xs space-y-1">
                        <div class="text-green-600 dark:text-green-400">Pago: R$
                            {{ number_format($totais->total_pago_funcionario ?? 0, 2, ',', '.') }}</div>
                        <div class="text-amber-600 dark:text-amber-400">Pendente: R$
                            {{ number_format($totais->total_pendente_funcionario ?? 0, 2, ',', '.') }}</div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 p-6">
                    ...
                    <div class="mt-2 text-xs space-y-1">
                        <div class="text-green-600 dark:text-green-400">Recebido: R$
                            {{ number_format($totais->total_pago_local ?? 0, 2, ',', '.') }}</div>
                        <div class="text-amber-600 dark:text-amber-400">Pendente: R$
                            {{ number_format($totais->total_pendente_local ?? 0, 2, ',', '.') }}</div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 p-6">
                    ...
                    @endphp
                    <div
                        class="text-2xl font-bold {{ $lucro >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
                        ...
                        <flux:table.rows>
                            @forelse ($porFuncionario as $item)
                                <flux:table.row>
                                    ...
                                    <flux:table.cell>{{ number_format($item->total_horas, 2, ',', '.') }}h
                                    </flux:table.cell>
                                    <flux:table.cell class="text-red-600 dark:text-red-400">R$
                                        {{ number_format($item->total_pagar, 2, ',', '.') }}</flux:table.cell>
                                    <flux:table.cell class="text-green-600 dark:text-green-400">R$
                                        {{ number_format($item->total_pago, 2, ',', '.') }}</flux:table.cell>
                                    <flux:table.cell class="text-amber-600 dark:text-amber-400">R$
                                        {{ number_format($item->total_pendente, 2, ',', '.') }}</flux:table.cell>
                                </flux:table.row>
                            @empty
                                <flux:table.row>
                                    <flux:table.cell colspan="5" class="text-center py-4">Nenhum registro encontrado.
                                    </flux:table.cell>
                                </flux:table.row>
                            @endforelse
                        </flux:table.rows>
                        ...
                        <flux:table.rows>
                            @forelse ($porLocal as $item)
                                ...
                                <flux:table.row>
                                    ...
                                    <flux:table.cell class="text-green-600 dark:text-green-400">R$
                                        {{ number_format($item->total_receber, 2, ',', '.') }}</flux:table.cell>
                                    <flux:table.cell class="text-green-600 dark:text-green-400">R$
                                        {{ number_format($item->total_pago, 2, ',', '.') }}</flux:table.cell>
                                    <flux:table.cell class="text-amber-600 dark:text-amber-400">R$
                                        {{ number_format($item->total_pendente, 2, ',', '.') }}</flux:table.cell>
                                    <flux:table.cell
                                        class="{{ $lucroLocal >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
                                        R$ {{ number_format($lucroLocal, 2, ',', '.') }}
                                    </flux:table.cell>
                                </flux:table.row>
                            @empty
                                <flux:table.row>
                                    <flux:table.cell colspan="6" class="text-center py-4">Nenhum registro encontrado.
                                    </flux:table.cell>
                                </flux:table.row>
                            @endforelse
                        </flux:table.rows>
